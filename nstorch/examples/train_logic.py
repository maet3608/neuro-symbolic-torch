...
from nstorch.models import NSModule, Gt, St, Xor, Or, And, Not
...
def gen_samples():
    for ha_up in range(0, 5):
        for ha_lo in range(0, 5):
            ...
            #yield 'Gt_1(x[0] + x[1])', [ha_up, ha_lo], 0.9 if is_grade0 else 0.1
            #yield 'Xor(Gt_2(x[0]), Gt_2(x[1]))', [ha_up, ha_lo], 0.9 if is_grade1 else 0.1
            #yield 'St_2(x[0] + x[1])', [ha_up, ha_lo], 0.9 if is_grade0 else 0.1

if __name__ == '__main__':
    device = 'cuda'
    modules = [Gt(1), Gt(2), St(2), Xor()]
